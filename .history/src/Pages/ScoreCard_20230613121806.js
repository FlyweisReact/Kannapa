/** @format */

import React, { useEffect } from "react";
import Navbar from "../Components/Layout/Navbar";
import img2 from "../images/c30.png";
import img8 from "../images/c27.png";
import img9 from "../images/c31.png";
import img10 from "../images/c32.png";
import Footer from "../Components/Layout/Footer";

const ScoreCard = () => {

    useEffect(() => {
        window.scrollTo(0,0)
    },[])

  return (
    <>
      <Navbar />

      <div className="Index_Center_Banner">
        <img src="./Images/1.png" alt="" />
      </div>

      <div className="Live_Score_Two_Sec ScoreCard_Two_Sec_Layout ">
        <div className="Left_Comp">
          <div className="Table_Comp">
            <div className="fourth2">
              <div className="fourth3">
                <img src={img2} alt="" />
                <h2>Bangladesh A vs West Indies A, Match 1</h2>
              </div>

              <div className="fourth3">
                <p>Series : </p>
                <p style={{ textDecoration: "underline" }}>
                  West Indies A tour of Bangladesh
                </p>
              </div>
            </div>

            <div className="fourth4">
              <div className="fourth5">
                <div className="fourth6">
                  <img src={img8} alt="" />
                  <h4>BAN</h4>
                </div>
                <h2>Yet to bat</h2>
              </div>
              <div className="fourth5">
                <div className="fourth6">
                  <img src={img9} alt="" />
                  <h4>WIN-A</h4>
                </div>
                <h2>417/6 (123)</h2>
              </div>
              <p className="fourthP">
                Stumps: Day2 West Indies A elected to bat
              </p>
            </div>

            <div className="fourth7">
              <table className="fourth8">
                <thead>
                  <tr>
                    <th>Batter</th>
                    <th>R</th>
                    <th>B</th>
                    <th>4s</th>
                    <th>6s</th>
                    <th>SR</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td>
                      <div className="fourth9">
                        <img src={img10} alt="" />
                        <div className="fourth10">
                          <h6>Lorem Ipsum</h6>
                          <p>c Jaker Ali b Musfik Hasan</p>
                        </div>
                      </div>
                    </td>
                    <td>83</td>
                    <td>236</td>
                    <td>7</td>
                    <td>0</td>
                    <td>35.17</td>
                  </tr>
                  <tr>
                    <td>
                      <div className="fourth9">
                        <img src={img10} alt="" />
                        <div className="fourth10">
                          <h6>Lorem Ipsum</h6>
                          <p>c Jaker Ali b Musfik Hasan</p>
                        </div>
                      </div>
                    </td>
                    <td>83</td>
                    <td>236</td>
                    <td>7</td>
                    <td>0</td>
                    <td>35.17</td>
                  </tr>
                  <tr>
                    <td>
                      <div className="fourth9">
                        <img src={img10} alt="" />
                        <div className="fourth10">
                          <h6>Lorem Ipsum</h6>
                          <p>c Jaker Ali b Musfik Hasan</p>
                        </div>
                      </div>
                    </td>
                    <td>83</td>
                    <td>236</td>
                    <td>7</td>
                    <td>0</td>
                    <td>35.17</td>
                  </tr>
                  <tr>
                    <td>
                      <div className="fourth9">
                        <img src={img10} alt="" />
                        <div className="fourth10">
                          <h6>Lorem Ipsum</h6>
                          <p>c Jaker Ali b Musfik Hasan</p>
                        </div>
                      </div>
                    </td>
                    <td>83</td>
                    <td>236</td>
                    <td>7</td>
                    <td>0</td>
                    <td>35.17</td>
                  </tr>
                  <tr>
                    <td>
                      <div className="fourth9">
                        <img src={img10} alt="" />
                        <div className="fourth10">
                          <h6>Lorem Ipsum</h6>
                          <p>c Jaker Ali b Musfik Hasan</p>
                        </div>
                      </div>
                    </td>
                    <td>83</td>
                    <td>236</td>
                    <td>7</td>
                    <td>0</td>
                    <td>35.17</td>
                  </tr>
                  <tr>
                    <td>
                      <div className="fourth9">
                        <img src={img10} alt="" />
                        <div className="fourth10">
                          <h6>Lorem Ipsum</h6>
                          <p>c Jaker Ali b Musfik Hasan</p>
                        </div>
                      </div>
                    </td>
                    <td>83</td>
                    <td>236</td>
                    <td>7</td>
                    <td>0</td>
                    <td>35.17</td>
                  </tr>
                </tbody>
              </table>
            </div>
          </div>

          <div className="Content_Div">
            <p className="head">Fall of Wickets</p>
            <div>
              <p>
                130-1 (Kirk McKenzie, 39.6 ov) , 160-2 (Raymon Reifer, 51.6 ov)
                , 281-3 (Tagenarine Chanderpaul, 82.2 ov) , 281-4 (Brandon King,
                82.6 ov) , 283-5 (Alick Athanaze , 83.6 ov) , 317-6 (Yannic
                Cariah, 99.5 ov)
              </p>
            </div>
          </div>


          <div className="Table_Comp">
            <div className="fourth7">
              <table className="fourth8">
                <thead>
                  <tr>
                    <th>BOWLER</th>
                    <th>O</th>
                    <th>M</th>
                    <th>R</th>
                    <th>W</th>
                    <th>NB</th>
                    <th>WD</th>
                    <th>ECO</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td>
                      <div className="fourth9">
                        <img src={img10} alt="" />
                        <div className="fourth10">
                          <h6>Musfik Hasan</h6>
                        </div>
                      </div>
                    </td>
                    <td>20</td>
                    <td>4</td>
                    <td>54</td>
                    <td>3</td>
                    <td>0</td>
                    <td>0</td>
                    <td>2.7</td>
                  </tr>
                  

                  <tr>
                    <td>
                      <div className="fourth9">
                        <img src={img10} alt="" />
                        <div className="fourth10">
                          <h6>Ripon Mondol</h6>
                        </div>
                      </div>
                    </td>
                    <td>22</td>
                    <td>7</td>
                    <td>61</td>
                    <td>1</td>
                    <td>0</td>
                    <td>0</td>
                    <td>2.77</td>
                  </tr>



                  <tr>
                    <td>
                      <div className="fourth9">
                        <img src={img10} alt="" />
                        <div className="fourth10">
                          <h6>Rejaur Rahman Raja</h6>
                        </div>
                      </div>
                    </td>
                    <td>20</td>
                    <td>4</td>
                    <td>54</td>
                    <td>3</td>
                    <td>0</td>
                    <td>0</td>
                    <td>2.7</td>
                  </tr>



                  <tr>
                    <td>
                      <div className="fourth9">
                        <img src={img10} alt="" />
                        <div className="fourth10">
                          <h6>Naeem Hasan</h6>
                        </div>
                      </div>
                    </td>
                    <td>20</td>
                    <td>4</td>
                    <td>54</td>
                    <td>3</td>
                    <td>0</td>
                    <td>0</td>
                    <td>2.7</td>
                  </tr>

                  <tr>
                    <td>
                      <div className="fourth9">
                        <img src={img10} alt="" />
                        <div className="fourth10">
                          <h6>Rishad Hossain</h6>
                        </div>
                      </div>
                    </td>
                    <td>20</td>
                    <td>4</td>
                    <td>54</td>
                    <td>3</td>
                    <td>0</td>
                    <td>0</td>
                    <td>2.7</td>
                  </tr>


                  <tr>
                    <td>
                      <div className="fourth9">
                        <img src={img10} alt="" />
                        <div className="fourth10">
                          <h6>Afif Hossain (c)</h6>
                        </div>
                      </div>
                    </td>
                    <td>20</td>
                    <td>4</td>
                    <td>54</td>
                    <td>3</td>
                    <td>0</td>
                    <td>0</td>
                    <td>2.7</td>
                  </tr>


                  <tr>
                    <td>
                      <div className="fourth9">
                        <img src={img10} alt="" />
                        <div className="fourth10">
                          <h6>Saif Hassan</h6>
                        </div>
                      </div>
                    </td>
                    <td>20</td>
                    <td>4</td>
                    <td>54</td>
                    <td>3</td>
                    <td>0</td>
                    <td>0</td>
                    <td>2.7</td>
                  </tr>
             
                  <tr>
                    <td>
                      <div className="fourth9">
                        <img src={img10} alt="" />
                        <div className="fourth10">
                          <h6>Mahmudul Hasan Joy</h6>
                        </div>
                      </div>
                    </td>
                    <td>20</td>
                    <td>4</td>
                    <td>54</td>
                    <td>3</td>
                    <td>0</td>
                    <td>0</td>
                    <td>2.7</td>
                  </tr>
             
                </tbody>
              </table>
            </div>
          </div>

        </div>

        <div className="Right_Comp">
          <div className="third8">
            <div className="third9">
              <button>LATEST NEWS</button>
            </div>

            <div className="third10">
              <h4>
                Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed
                diam nonumy eirmod tempor invidunt ut
              </h4>
              <p>43 minutes ago</p>
            </div>
            <hr className="horizontal2" />
            <div className="third10">
              <h4>
                Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed
                diam nonumy eirmod tempor
              </h4>
              <p>43 minutes ago</p>
            </div>
            <hr className="horizontal2" />
            <div className="third10">
              <h4>
                Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed
                diam
              </h4>
              <p>an hour ago</p>
            </div>
            <div className="third10">
              <h4>
                Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed
                diam{" "}
              </h4>
              <p>3 hours ago</p>
            </div>
            <hr className="horizontal2" />

            <h3 className="third11">More News...</h3>
          </div>

          <div className="third12">
            <img src="./Images/88.png" alt="" />
          </div>

          <div className="third13">
            <div className="third14">
              <button>FEATURED VIDEOS</button>
            </div>

            <div className="third15 HIde_THid_One ">
              <img src="./Images/8.png" alt="" />
              <h4>
                CSK vs DC Dream 11 Prediction | Chennai Super Kings vs Delhi
                Capitals Dream 11 | IPL 2023
              </h4>
              <p>May 10,2023 8:39 AM</p>
            </div>

            <div className="third15">
              <img src="./Images/90.png" alt="" />
              <h4>
                Rashid Khan & Noor Ahmad The Best Spin Duo In IPL 2023? | The
                Afghani Spin Wizards Of Gujarat Titans
              </h4>
              <p>May 9, 2023 6:42 PM</p>
            </div>

            <div className="third15">
              <img src="./Images/10.png" alt="" />
              <h4>
                Rajasthan Royal's Recent Form - A Concern For Sanju Samson |
                What's Going Wrong For Rajasthan Royals
              </h4>
              <p>May 10,2023 8:39 AM</p>
            </div>
            <div className="third15">
              <img src="./Images/8.png" alt="" />
              <h4>
                CSK vs DC Dream 11 Prediction | Chennai Super Kings vs Delhi
                Capitals Dream 11 | IPL 2023
              </h4>
              <p>May 10,2023 8:39 AM</p>
            </div>

            <div className="third15">
              <img src="./Images/90.png" alt="" />
              <h4>
                Rashid Khan & Noor Ahmad The Best Spin Duo In IPL 2023? | The
                Afghani Spin Wizards Of Gujarat Titans
              </h4>
              <p>May 9, 2023 6:42 PM</p>
            </div>

            <div className="third15">
              <img src="./Images/10.png" alt="" />
              <h4>
                Rajasthan Royal's Recent Form - A Concern For Sanju Samson |
                What's Going Wrong For Rajasthan Royals
              </h4>
              <p>May 10,2023 8:39 AM</p>
            </div>
          </div>
        </div>
      </div>

      <Footer />
    </>
  );
};

export default ScoreCard;
